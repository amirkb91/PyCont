...
def initialguess_eig():
    ...
    eigfile = "/home/akb110/Codes/beam_cpp/examples/cantilever/cantilever_eig.h5"
    eigdata = h5py.File(eigfile, "r")
    eig = np.array(eigdata["/Eigenvectors"])
    frq = eigdata["/Frequencies"]
    eig[np.abs(eig) < 1e-10] = 0.0

    nnm = 1
    scale = 5e-3
    x0 = eig[:, nnm - 1] * scale
    x0 = x0[3:]  # remove boundary nodes
    v0 = np.zeros(len(x0))
...
def zerofunction(T, X, par):
    ...
    path2cpp = "/home/akb110/Codes/beam_cpp/examples/cantilever/"
    exec_cpp = "/home/akb110/Codes/beam_cpp/cmake-build-release/examples/cantilever"
    paramfile = "parameters.json"
    eigenfile = "cantilever_eig"
    ouputfile = "cantilever_out"
    ...
    inc = np.pad(inc, (3, 0), "constant")
    vel = np.pad(vel, (3, 0), "constant")
    ...
    if cvg:
        ...
        H = np.concatenate([inc_out[3:], vel_out[3:]])
        Mm0 = outputdata["/Sensitivity/Monodromy_m0"][:]
        dHdt = nperiod * np.concatenate([Mm0[3:ndof_all, -1], Mm0[ndof_all + 3 :, -1]])
        Mm0 = np.delete(Mm0, -1, axis=1)  # stored inside dHdt so delete
        Mm0 = np.block(
            [
                [Mm0[3:ndof_all, 3:ndof_all], Mm0[3:ndof_all, ndof_all + 3 :]],
                [Mm0[ndof_all + 3 :, 3:ndof_all], Mm0[ndof_all + 3 :, ndof_all + 3 :]],
            ]
        )
