...
class Frame:
    ...
    def relative_frame(cls, n_dim, frame_a, frame_b):
        ...
        if n_dim == 2:
            q = UnitQuaternion.relative_rotation(n_dim, frame_a[:2], frame_b[:2])
            x = UnitQuaternion.rotateT_vec(n_dim, frame_a[:2], frame_b[2:] - frame_a[2:])
        elif n_dim == 3:
            q = UnitQuaternion.relative_rotation(n_dim, frame_a[:4], frame_b[:4])
            x = UnitQuaternion.rotateT_vec(n_dim, frame_a[:4], frame_b[4:] - frame_a[4:])
