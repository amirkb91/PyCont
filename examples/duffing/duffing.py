...
class Duffing:
    ...
    def model_sens_ode(cls, t, ic, T):
        ...
        f = cls.delta * xdot + cls.alpha * x + cls.beta * x**3
        force = cls.F * np.cos(2 * np.pi / T * t + cls.phi)
        dforce_dT = cls.F * np.sin(2 * np.pi / T * t + cls.phi) * 2 * np.pi * t / T**2
    ...
    # @classmethod
    # def periodsens_centdiff(cls, X0, T):
    #     eps = 1e-8
    #     Tplus = T + eps
    ...
    # @classmethod
    # def forcesens_centdiff(cls, X0, t):
    #     F = cls.F
    #     T = t[-1]
    #     eps = 1e-8
    #     Fplus = F + eps
    #     cls.F = Fplus
    #     XTplus = np.array(odeint(cls.model_ode, X0, t, args=(T, ), tfirst=True))[-1, :]
    #     Fmins = F - eps
    #     cls.F = Fmins
    #     XTmins = np.array(odeint(cls.model_ode, X0, t, args=(T, ), tfirst=True))[-1, :]
